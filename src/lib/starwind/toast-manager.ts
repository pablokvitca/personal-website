...
export class StarwindToastManager {
  ...
  private handleFocusout(e: FocusEvent): void {
    ...
    const rect = this.viewport.getBoundingClientRect();
    const lastMouse = window.__lastMousePosition;
    const isMouseInside =
      lastMouse &&
  ...
  public destroy(): void {
    ...
    if (window.__starwind__?.toast === this) {
      window.__starwind__.toast = null;
    }
  }
...
export function setupToasts(currentManager: StarwindToastManager | null): StarwindToastManager | null {
  ...
  const manager = new StarwindToastManager(viewport);
  window.__starwind__ = window.__starwind__ ?? {};
  window.__starwind__.toast = manager;
  return manager;
}
